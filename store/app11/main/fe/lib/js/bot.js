...
_cp.on.show_adminbot = function(){
	...
	$("#btnListen").unbind().bind('click', function(e) {_cp.on.bot_on_ask();});

	//$('#btnListen').click(function () {bot_on_ask();});
	
	//$('#btnSubmit').click(function () {submit_speech();});
	$('#btnSubmit').click(function () {_cp.on.bot_on_submit();});
	
	//$('#btnClear').click(function () {clear_speech();});
	$('#btnClear').click(function () {_cp.on.bot_on_start();});

	$('#btnSpeak').click(function () {_cp.on.bot_on_speak();});
	$('#btnReset').click(function () {_cp.on.bot_on_set();});
	
	$('#speak_lang').change(function () {_cp.on.change_lang();});
	$('#listen_lang').change(function () {_cp.on.switch_lang();});
...
_cp.on.show_userbot = function(){
	...
	ask_url = `	
		<div class="my-2">
			<div class="btn-group" role="group" aria-label="Basic example">
			  <button type="button" class="btn btn-primary" id="btnListen">Ask</button>
			  <button type="button" class="btn btn-primary ms-2" id="btnSubmit">Submit</button>
			</div>
			<div class="btn-group float-end" role="group" aria-label="Basic example">
			  <select id="listen_lang" class="form-select" style="width:260px">
				<option value="en-IN" selected>English-India</option>
		...
		<div class="my-2">
			...
			<div class="btn-group float-end" role="group" aria-label="Basic example" id="sett">
			  <select id="speak_lang" style="max-width:250px" class="form-select">
 ...
 _cp.on.switch_lang = function(){	
	_cp.on.set_voice($('#speak_lang'),$('#listen_lang').val());	
}

_cp.on.bot_on_start = function(){
...
_cp.on.bot_on_submit = function(){	
	...
	if ($('#listen_lang').val() !== 'en-IN'){
		in_lang = _cp.on.get_input_lang();
		j_dat = {from_lang: in_lang, to_lang: 'en', txt_body : $('#final_span').text()}
		console.log(JSON.stringify(j_dat))		
		_app.post("/app04/api/mod03/convs/translate",JSON.stringify(j_dat),function (result) {
		   console.log(result);
		   _cp.on.call_GPT(result, $('#final_span').text());
		   //speech_text['en'] = result
		});			
	}
	else {
		_cp.on.call_GPT($('#final_span').text(), $('#final_span').text());
	}
}

_cp.on.call_GPT = function(q_text, l_text){
