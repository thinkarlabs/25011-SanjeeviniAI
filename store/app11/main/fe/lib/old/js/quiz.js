...
_cp.on.showQnA = function(){
	...
	qna_url = `
		<div class="my-2">
			...
			<button type="button" class="btn btn-primary ms-1" id="btnStart">Start Quiz</button>
			  ...
			  <h3 id="lbl_pos" class="ms-2 mt-2"> 0 / 3</h3>
			</div>
	...
	$("#btnStart").unbind().bind('click', function(e) {quiz_on_start();});
	$("#btnSpeak").unbind().bind('click', function(e) {quiz_on_ask();});
	$("#btnNext").unbind().bind('click', function(e) {quiz_on_next();});
	
	$("#btnListen").unbind().bind('click', function(e) {quiz_on_answer();});
	$("#btnSubmit").unbind().bind('click', function(e) {quiz_on_submit();});
	$("#btnClear").unbind().bind('click', function(e) {quiz_on_clear();});
	
	$('#listen_lang').change(function () {switch_quiz_lang();});
	quiz_on_init();
	//$('#btnListen').click(function () {listenQnA();});
}

function switch_quiz_lang(){	
...
function quiz_on_start(){
	...
	if (testMode){		
		e_list= [{
			...
			if (lang !== 'English'){				
				$.ajax({
				   url: "/app04/api/student/topics/translate-quiz", 
				   type: "POST",
				   dataType: "json",
...
function quiz_eval(user_response_en){
